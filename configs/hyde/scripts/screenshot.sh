...
USAGE() {
	...
	Options:
		p             Print all outputs (copy to clipboard)
		s             Select area or window to screenshot (copy to clipboard)
		sf            Select area or window with frozen screen (copy to clipboard)
		m             Screenshot focused monitor, then annotate
		sc            Use tesseract to scan image, then add to clipboard
		sq            Perform QR scan on image
...
take_screenshot() {
    ...
    shift
    local extra_args=("$@")
    local target_file="$save_dir/$save_file"
...
copy_screenshot() {
    ...
    command=("$LIB_DIR/hyde/screenshot/grimblast" "${extra_args[@]}" "copy" "${mode}")
    print_log -g "Executing screenshot command: ${command[*]}"
    if eval "${command[*]}"; then
        [[ "${SCREENSHOT_NOTIFY}" != false ]] && send_notifs -r 9 -a "HyDE Alert" "screenshot copied to clipboard"
    else
        send_notifs -a "HyDE Alert" "Screenshot Error" "Failed to take screenshot"
        return 1
    fi
...
case $1 in
p | printscreen) copy_screenshot "screen" ;;
s | snip) copy_screenshot "area" ;;
sf | snapfreeze) copy_screenshot "area" "--freeze" ;;
